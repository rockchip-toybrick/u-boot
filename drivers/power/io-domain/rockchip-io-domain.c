				   ...
				   int uV)
{
	...
	ret = regmap_write(priv->grf, priv->sdata->grf_offset, val);
	if (ret) {
		dev_err(priv->dev, "Couldn't write to GRF\n");
		return ret;
	}

	printf("rockchip_iodomain_write: write 0x%x to 0x%llx\n",
	       val, priv->grf->base + priv->sdata->grf_offset);
	return 0;
}
...
{
	char supply_name[32]; /* 32 is max size of property name */
	struct udevice *supply_udev;
	ofnode supplynode;
	u32 phandle;
	int ret;

	snprintf(supply_name, 32, "%s-supply", supply);

	/* Get regulator and clk */
	if (!ofnode_read_u32(node, supply_name, &phandle)) {
		supplynode = ofnode_get_by_phandle(phandle);
		ret = regulator_get_by_devname(supplynode.np->name, &supply_udev);
		if (ret) {
			printf("IO-DOMAIN: %s: Get supply(%s) failed, ret=%d",
				   supply_udev->name, supplynode.np->full_name, ret);
			return NULL;
		}
		debug("IO-DOMAIN: supply: %s\n", supplynode.np->full_name);
	}
...
static int rockchip_iodomain_probe(struct udevice *dev)
{
	...
	for (i = 0; i < MAX_SUPPLIES; i++) {
		...
		if (uV < 0) {
			printf("cound not get voltage from %s\n", reg->name);
			continue;
		}
